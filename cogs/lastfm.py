...
def make_recent_tracks_list(parsed_content):
    ...
    for track in recent_tracks:
        ...
        recent_tracks_list.append((name, artist, image))
...
class LastfmCog:
    ...
    async def lastfm(self, ctx):
        ...
        bad_permissions = "Sorry, you cannot use the lastfm commands here."
    ...
    async def embed_last_played(self, ctx):
        ...
        name, artist, image = last_played

        rym_link = ("https://rateyourmusic.com/" +
                    "search?&searchtype=a&searchterm={}")

        rym_link = rym_link.format(artist.replace(" ", "%20"))
        artist_search_url = ("[{}]({})").format(artist, rym_link)
        user_search_url = "https://www.last.fm/user/{}".format(user)

        embed = discord.Embed(colour=0xFF0000, title=name,
